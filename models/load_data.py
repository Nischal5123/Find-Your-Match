...
class ToTensor():
    ...
    def __call__(self, DataFrame):
        ...
        if self.is_mlp:
            pairs1, pairs2, label = torch.tensor(DataFrame.iloc[:, :2048].values), torch.tensor(
                DataFrame.iloc[:, 2048:-1].values), torch.tensor(DataFrame.iloc[:, -1].values)
        # Contains tokenised SMILES
        else:
            pairs1, pairs2, label = torch.tensor(DataFrame.iloc[:, 0], dtype=torch.long), torch.tensor(
                DataFrame.iloc[:, 1], dtype=torch.long), torch.tensor(DataFrame.iloc[:, 2])
